...
!!
      Rmass = Qmass * BEAM(NEL)%PAR%Length**2 / 12.0d0
!!
      DO i = 3,4
        NODE(BEAM(NEL)%PAR%IX(i))%Mass =                                       &
...
!!
        ...
        DTbd = BEAM(NEL)%RES%Length * BEAM(NEL)%RES%Length * 0.5d0 /             &
...
!!
      ...
      Fy = Ry * Fr + Sy * Fs + Ty * Ft
      Fz = Rz * Fr + Sz * Fs + Tz * Ft
      Srs = (0.5d0 * Volume) * (MEAN%Nrs + BEAM(NEL)%RES%Trs * S_Factor)
      Srt = (0.5d0 * Volume) * (MEAN%Nrt + BEAM(NEL)%RES%Trt * T_Factor)
      S1 = Sx * Srt - Tx * Srs
      S2 = Sy * Srt - Ty * Srs
      S3 = Sz * Srt - Tz * Srs
...
!!
      IF (Isec .EQ. 5) THEN
        ...
        IF (Elastic_Material) THEN
          ...
          ENDDO
          Awt(7) = Awgt(7,3) * ((Height-2.0d0*Tflange)*Twall/(6.0*Area))
          Awt(8) = Awgt(8,3) * ((Height-2.0d0*Tflange)*Twall/(6.0*Area))
          Awt(9) = Awgt(9,3) * ((Height-2.0d0*Tflange)*Twall/(6.0*Area))
          DO i = 1,Npts
            Bwt(i) = Awt(i)
          ENDDO
        ELSE
          ...
          ENDDO
          Awt( 7) = Bwgt( 7,3) * ((Height-2.0d0*Tflange)*Twall/(6.0*Area))
          Awt( 8) = Bwgt( 8,3) * ((Height-2.0d0*Tflange)*Twall/(6.0*Area))
          Awt( 9) = Bwgt( 9,3) * ((Height-2.0d0*Tflange)*Twall/(6.0*Area))
          Awt(14) = Bwgt(14,3) * ((Height-2.0d0*Tflange)*Twall/(6.0*Area))
          Awt(15) = Bwgt(15,3) * ((Height-2.0d0*Tflange)*Twall/(6.0*Area))
          DO i = 1,Npts
            Bwt(i) = Awt(i)
          ENDDO
        ENDIF
...
!!
      DO i = 1,Npts
        S(i) = (0.5d0 * Width  - Sos)
        T(i) = (0.5d0 * Height - Tos)
      ENDDO
      IF (Isec .EQ. 5) THEN
        T( 7) = (0.5d0 * Height - Tflange)
        T( 8) = (0.5d0 * Height - Tflange)
        T( 9) = (0.5d0 * Height - Tflange)
        T(14) = (0.5d0 * Height - Tflange)
        T(15) = (0.5d0 * Height - Tflange)
      ENDIF
      DO i = 1,Npts
...
!!
      Ers = -(0.5d0 * (Wt - Vsr) * S_Factor)
      Ert =  (0.5d0 * (Ws + Vtr) * T_Factor)
      DO i = 1,Npts
        Drs(i) = -T(i) * (0.5d0 * Wrr)
        Drt(i) = +S(i) * (0.5d0 * Wrr)
      ENDDO
...
!! Local variables.
!!
!!      DATA    Root2   /1.414213562/           ! Root2 = SQRT (2.0d0)
!!      DATA    Root6   /2.449489743/           ! Root6 = SQRT (6.0d0)
!!      DATA    Third   /0.33333333333333/      ! Third = 1.0d0 / 3.0d0
!!      DATA    Troot   /0.81649658092773/      ! Troot = SQRT (2.0d0/3.0d0)
!!
...
!!
        Psi = 2.0d0*(Third*(Axial*Axial) + (Shear*Shear))
!!
...
!!
        IF (QP .NE. 0.0) THEN
          ED = 4.0*(Shear*Drc(i)) + 2.0d0*Third*(Axial*Drr(i))
          IF (ED .GT. 0.0) THEN
...
!!
 ...
 100      CONTINUE
        ...
        IF ((F1-F2) .GT. Epsilon*F2) THEN
          KOUNT = KOUNT + 1
          DF1 = -2.0d0 * ((A1/(B2*C2))*A2 + (A3/(B4*C4))*A4)
          DF2 =  2.0d0 * A7*(A5*A6 + dL*A5*DF1/A6)
          dL = dL - (F1-F2)/(DF1-DF2)
     ...
     &      )
        ...
        D2 = Root6 / B2
        D4 = Root2 / B4
        BEAM(NEL)%RES%Axial(i) = Axial * 0.5d0 * (D2 + D4)
...
!!
      MatID = BEAM(NEL)%PAR%MatID
      YM = MATERIAL(MatID)%PVAL(6)
      SM = 0.5d0 * (MATERIAL(MatID)%PVAL(12) + MATERIAL(MatID)%PVAL(13))
!!
!! Define (rho c**2) for longitudinal (RCL2) and shear waves (RCS2).
!!
      SOUND_SPEED%RCL2 = YM
