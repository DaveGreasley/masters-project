...
!!
!SPEC_omp2001        CALL PLATQ_MASS ( NEL,SecID,MatID )
!!
...
!$OMP CRITICAL
        ...
        ENDDO
        DO i = 5,8
          NODE(PLATQ(NEL)%PAR%IX(i))%Mass = NODE(PLATQ(NEL)%PAR%IX(i))%Mass + RMass
        ENDDO
!$OMP END CRITICAL
!!
        MATERIAL(MatID)%Mass = MATERIAL(MatID)%Mass + QMass*4.0d0
      DO i = 1,4
...
!!
!! Local variables.
      INTEGER            :: SecID,MatID,Imax
      INTEGER            :: SecIDa,MatIDa,Imaxa
      INTEGER            :: IX(8)
...
!!
!bodo!!$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(N,IX,i)
        DO N = 1,NUMP4
...
!bodo!!$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(N,i,IX)
        DO N = 1,NUMP4
          IX(1:8) = PLATQ(N)%PAR%IX(1:8)
          DO i = 1,8
!bodo!!$OMP CRITICAL
...
!!
      ...
      ENDIF
!!
!! Compute nodal translational mass.
!!
!$OMP PARALLEL  &
!$OMP   DEFAULT(SHARED), PRIVATE(N,i,L,Imax,RMass,QMass,MatID,SecID, &
!$OMP Area,Thickness) 
!$OMP DO
!!
        DO N = 1,NUMNP
          Imax = ELEMENTS_PER_NODE(N)
            ...
            DO i = 1,Imax
              L = ELEMENT_INDEX(i,N)
              MatID = PLATQ(L)%PAR%MatID
              SecID = PLATQ(L)%PAR%SecID
              QMass = QMass + MATERIAL(MatID)%PVAL(1)*PLATQ(L)%PAR%Area*SECTION_2D(SecID)%Thickness/4.0D+0
            ENDDO
            NODE(N)%Mass = NODE(N)%Mass + QMass
        ENDDO
!$OMP END DO NOWAIT
!!
        ...
        DO N = NUMNP+1,NUMRT
          Imax = ELEMENTS_PER_NODE(N)
            ...
            DO i = 1,Imax
              ...
              MatID = PLATQ(L)%PAR%MatID
              SecID = PLATQ(L)%PAR%SecID
              Area = PLATQ(L)%PAR%Area
              Thickness = SECTION_2D(SecID)%Thickness
              QMass = MATERIAL(MatID)%PVAL(1)*Area*Thickness/4.0D+0
              RMass = RMass + QMass*(Area+Thickness**2)/12.0D+0
            ENDDO
            NODE(N)%Mass = NODE(N)%Mass + RMass
        ENDDO
!$OMP END DO
!$OMP END PARALLEL
!!
