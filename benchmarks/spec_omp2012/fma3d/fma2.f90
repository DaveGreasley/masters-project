...
!!
      ERRORC = 0
!$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(N,ERRMSG) REDUCTION(+:ERRORC)
      DO N = 1,NUMRT
...
!!
! SPEC OMP2001      IF (TIMSIM%Total .GE. PRINT%Time) THEN
      IF (mod(TIMSIM%Step,10) .EQ. 0) THEN
!!
...
!!
      ...
      END
!!_
      SUBROUTINE xxSCATTER_ELEMENT_NODAL_FORCES
!!
...
!!
 ...
 100    CONTINUE
        ...
        FIRST = .TRUE.
        NEXT_NBGN = NUMP4 + 1
!!!!$OMP PARALLEL DO PRIVATE (IX, UNCOUPLED, N) ORDERED
...
!!_
      ...
      SUBROUTINE SCATTER_ELEMENT_NODAL_FORCES_M
!!
!! Copyright (c) by KEY Associates, 3-NOV-1991 21:15:04
!!
      USE shared_common_data
...
!! Local variables.
      INTEGER             :: IX(8)
      LOGICAL, SAVE      :: FIRST = .TRUE.
      INTEGER, PARAMETER :: N1(8) = (/1,1,1,1,1,1,1,1/)
...
!!
!! Convert scatter from elements into gather by nodal points.
!!
      IF (FIRST .AND. NUMP4.GT.0) THEN
!!
!! Allocate storage for count of number of elements contributing to
!! each nodal point.
!!
        ALLOCATE (ELEMENTS_PER_NODE(1:NUMRT), STAT=IALLOC_FLAG)
        CALL REPORT_ALLOCATION_EVENT (IALLOC_FLAG,'ELEMENTS_PER_NODE',NUMRT)
!$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(N)
        DO N = 1,NUMRT
          ELEMENTS_PER_NODE(N) = 0
        ENDDO
!$OMP END PARALLEL DO
!!
!! Count number of elements that contibute to each nodal point.
!!
!sam!$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(N,IX,i)
        DO N = 1,NUMP4
          IX(1:8) = PLATQ(N)%PAR%IX(1:8)
          DO i = 1,8
!sam!$OMP ATOMIC
            ELEMENTS_PER_NODE(IX(i)) = ELEMENTS_PER_NODE(IX(i)) + 1
          ENDDO
        ENDDO
!sam!$OMP END PARALLEL DO
!!
!! Find maximum number of elements at any node.
!!
        MAX_PER_NODE = 0
!$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(N) REDUCTION(MAX:MAX_PER_NODE)
        DO N = 1,NUMRT
          MAX_PER_NODE = MAX (MAX_PER_NODE, ELEMENTS_PER_NODE(N))
        ENDDO
!$OMP END PARALLEL DO
!!
!! Allocate nodal point come-from index lists.
!!
        MXPN = MAX_PER_NODE

        ALLOCATE (ELEMENT_INDEX(1:MXPN,1:NUMRT), STAT=IALLOC_FLAG)
        CALL REPORT_ALLOCATION_EVENT (IALLOC_FLAG,'ELEMENT_INDEX',MXPN*NUMRT)
...
!$OMP END PARALLEL DO
        ...
        ALLOCATE (NODE_PT_INDEX(1:MXPN,1:NUMRT), STAT=IALLOC_FLAG)
        CALL REPORT_ALLOCATION_EVENT (IALLOC_FLAG,'NODE_PT_INDEX',MXPN*NUMRT)
!$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(N,i)
        DO N = 1,NUMRT
          DO i = 1,MXPN
            NODE_PT_INDEX(i,N) = 0
...
!sam!$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(N,i,IX)
        DO N = 1,NUMP4
          IX(1:8) = PLATQ(N)%PAR%IX(1:8)
          DO i = 1,8
!sam!$OMP CRITICAL
            ELEMENT_INDEX(:,IX(i)) = (/ N, ELEMENT_INDEX(1:MXM1,IX(i)) /)
            NODE_PT_INDEX(:,IX(i)) = (/ i, NODE_PT_INDEX(1:MXM1,IX(i)) /)
!sam!$OMP END CRITICAL
          ENDDO
        ENDDO
!sam!$OMP END PARALLEL DO
!!
...
!!
      ...
      ENDDO
!!
!! Gather nodal point internal forces from PLATQ elements.
!!
      IF (NUMP4 .GT. 0) THEN
!$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(N,Imax,i,L,K,Xint,Yint,Zint)
        DO N = 1,NUMRT
          Imax = ELEMENTS_PER_NODE(N)
          IF (Imax .GT. 0) THEN
            Xint = 0.0
            Yint = 0.0
            Zint = 0.0
            DO i = 1,Imax
              L = ELEMENT_INDEX(i,N)
              K = NODE_PT_INDEX(i,N)
              Xint = Xint + PLATQ(L)%RES%Xint(K)
              Yint = Yint + PLATQ(L)%RES%Yint(K)
              Zint = Zint + PLATQ(L)%RES%Zint(K)
            ENDDO
            FORCE(N)%Xint = FORCE(N)%Xint + Xint
            FORCE(N)%Yint = FORCE(N)%Yint + Yint
            FORCE(N)%Zint = FORCE(N)%Zint + Zint
          ENDIF
        ENDDO
!$OMP END PARALLEL DO
      ENDIF
...
!$OMP DO PRIVATE(NDC,N,SetID, Code, Rotation) 
        DO NDC = 1,NUMDC
...
!$OMP SHARED(DISPLACEMENT_BC, NODE, NUMDC, DTR, MOTION, FORCE, TIMSIM)
      DO 200 NDC = 1,NUMDC
        SetID = DISPLACEMENT_BC(NDC)%SetID
        Code  = DISPLACEMENT_BC(NDC)%Code
...
!!
!! Local variables.
      INTEGER :: M,MODE,Istart,Iend
      COMMON /NEXT_NP_ID_SAVE/ M,MODE,Istart,Iend
!$OMP THREADPRIVATE (/NEXT_NP_ID_SAVE/)
!!
...
!!
      IF (NUMP4 .GT. 0 .AND. PRINT%PLATQ .LT. 0) THEN
        ...
        DO N = 1,NUMP4,IPI
          ...
          DO i = 1,Ipts
            m = Ist + (i-1)
            WRITE (IO_UNIT%LCRO,156)                                           &
     &        PLATQ(N)%PAR%EleID,                                              &
     &        i,                                                               &
     &        Stress(1,m),Stress(2,m),Stress(4,m),                 &
     &        MATERIAL(MatID)%MatID,                                           &
     &        MATERIAL(MatID)%Type,                                            &
     &        PLATQ(N)%RES%Int_Eng
          ENDDO
...
!!
 ...
 115    FORMAT                                                                 &
     &    (                                                                    &
     &    1X,I8,1X,9(1PE13.4),3X                                            &
     &    )
 116    FORMAT                                                                 &
     &    (                                                                    &
     &    1X,I8,1X,3(1PE13.4),3X                                            &
     &    )
 125    FORMAT                                                                 &
     &    (                                                                    &
     &    I9,6(1PE14.4),1X,I4,I8,1PE11.2                                   &
     &    )
 135    FORMAT                                                                 &
     &    (                                                                    &
     &    I9,6(1PE14.4),1X,I4,I8,1PE11.2                                    &
 ...
 146    FORMAT                                                                 &
     &    (                                                                    &
     &    I9,1(1PE14.4),1X,I4,I8,1PE11.2                                    &
     &    )
 155    FORMAT                                                                 &
     &    (                                                                    &
     &    I8,'/',I1,6(1PE14.4),1X,I4,I8,1PE11.2                             &
     &    )
 156    FORMAT                                                                 &
     &    (                                                                    &
     &    I8,'/',I1,3(1PE14.4),1X,I4,I8,1PE11.2                             &
     &    )
 165    FORMAT                                                                 &
     &    (                                                                    &
     &    I8,'/',I2,6(1PE14.4),1X,I4,I8,1PE11.2                             &
     &    )
 166    FORMAT                                                                 &
     &    (                                                                    &
     &    I8,'/',I2,4(1PE14.4),1X,I4,I8,1PE11.2                             &
     &    )
 175    FORMAT                                                                 &
     &    (                                                                    &
     &    I9,6(1PE14.4),1X,I4,I8,1PE11.2                                    &
 ...
 215    FORMAT                                                                 &
     &    (                                                                    &
     &    I9,6(1PE14.4),3X,A,4X                                             &
     &    )
 225    FORMAT                                                                 &
     &    (                                                                    &
     &    I9,6(1PE14.4),2X,A,4X                                             &
     &    )
 235    FORMAT                                                                 &
     &    (                                                                    &
     &    I9,4(1PE14.4),2X,A,4X                                             &
     &    )
 245    FORMAT                                                                 &
     &    (                                                                    &
     &    I9,3(1PE14.4),1X,A,4X                                             &
     &    )
!!
      END
