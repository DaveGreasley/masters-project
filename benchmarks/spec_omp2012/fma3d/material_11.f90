...
!!
      IF (QP .NE. 0.0d0 .AND. QD .NE. 0.0d0) THEN
        Edot = Xi * Drr
        IF (Edot .GT. 0.0d0) THEN
...
!!
      IF (CONTROL%SUBCYC .GT. 0 .OR. Einc .EQ. 0.0d0) THEN
        SOUND_SPEED%RCL2 = QE
        SOUND_SPEED%RCS2 = 0.0d0
      ELSE
...
!!
      LOCAL_STRESS(1) = Srr
      LOCAL_STRESS(2) = Sss
      LOCAL_STRESS(3) = 0.0d0
      LOCAL_STRESS(4) = Srs
      LOCAL_STRESS(5) = 0.0d0
      LOCAL_STRESS(6) = 0.0d0
!!
!! Internal energy.
!!
...
!!
      ...
      REAL(KIND(0D0)), PARAMETER :: Epsilon = 1.0D-7
      INTEGER,         PARAMETER :: KMAX    = 128
...
!!
 ...
 100  CONTINUE
      ...
      A7 = AK + A5*(dL*A6)
      F2 = A7*A7
      IF (ABS(F1-F2) .GT. Epsilon*F2) THEN
        KOUNT = KOUNT + 1
