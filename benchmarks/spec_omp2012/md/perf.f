      ...
      case(0)
         do 100 iproc = 0,nprocs-1
            ...
            if(iproc.lt.nprocs-1) then
               call MPI_recv(xbuff,18,MPI_DOUBLE_PRECISION,iproc+1,1,MPI_COMM_WORLD,status,ierror)
               call MPI_recv(ibuff,9,MPI_INTEGER,iproc+1,1,MPI_COMM_WORLD,status,ierror)
            endif
  ...
  100    continue
      ...
      case default
        !call MPI_recv(token,1,MPI_INTEGER,myrank-1,1,MPI_COMM_WORLD,status,ierror)
         call MPI_send(xbuff,18,MPI_DOUBLE_PRECISION,0,1,MPI_COMM_WORLD,ierror)
         call MPI_send(ibuff,9,MPI_INTEGER,0,1,MPI_COMM_WORLD,ierror)
