...
c	/* Bi-CGSTAB */
      ...
      real*8 b(nb,nx,ny,nzl), a(nb,nb,nx,ny,nzl)
      real*8 axp(nb,nb,nx,ny,nzl), ayp(nb,nb,nx,ny,nzl),
     1       azp(nb,nb,nx,ny,nzl)
      real*8 axm(nb,nb,nx,ny,nzl), aym(nb,nb,nx,ny,nzl),
     1       azm(nb,nb,nx,ny,nzl)
      real*8 x(nb,nx,ny,nzl+2)
      real*8 p(nb,nx,ny,nzl+2), r(nb,nx,ny,nzl+2),
     1       rhat(nb,nx,ny,nzl+2)
      real*8 v(nb,nx,ny,nzl+2), t(nb,nx,ny,nzl+2)

      real*8 epsilon,r2,alpha,beta,rho,oldrho,omega,oldomega
      real*8 r_r,target,tmp,tmp1
      real*8 rh_r,rh_v,t_r,t_t
...
!$OMP END PARALLEL DO
	...
	write (10,'(2(1X,G20.11))') r_r,epsilon
...
c       Compute global r * r norm
	...
	write (10,'(1X,A,I6,A)')'convergence after ',kit,' iterations.'
	...
	real*8 y(nb,nx,ny,nzl+2),x(nb,nx,ny,nzl+2)

	real*8 a(nb,nb,nx,ny,nzl),
